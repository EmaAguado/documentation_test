...
class MayaUnrealTestPlugin(ShotgridPlugin):
    TITLE = "MayaUnrealTest"
    ...
    uses_deadline = True
    bdlregex = r"mut_(\d\d\d_|[a-zA-Z]{2}_([a-zA-Z09]*._){2})bd[lw]_[vw]\d\d\d\.xlsx"

    def __new__(cls: "MayaUnrealTestPlugin", *args, **kwargs) -> "MayaUnrealTestPlugin":
        cls._local_root = Path(cls.env_handler.get_env("MAYAUNREALTEST_LOCAL_PATH"))
        cls._server_root = Path(
            cls.env_handler.get_env(
                "MAYAUNREALTEST_SERVER_PATH",
                Path(
                    "\\\\qsrv01.mondotvcanarias.lan\\demoproj_maya_unreal\\MayaUnrealTest"
                ),
            )
        )
        ...
        cls.SL_SERVER_PATH = Path(cls._server_root, "production/publish/library/anim_lib")
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._naming_regex = [
            r"^mut_[a-z]{2}_[A-Za-z0-9]+_[A-Za-z0-9]+_[a-z]+_[wv]\d\d\d.*",
            r"^mut(_\d\d\d)_([a-z]+)_[wv]\d\d\d.*",
            r"^mut(_\d\d\d){2}_([a-z]+)_[wv]\d\d\d.*",
            r"^mut(_\d\d\d){3}_([a-z]+)_[wv]\d\d\d.*",
            r"^mut(_\d\d\d){3}(_[a-z]+){1,3}_[wv]\d\d\d.*",
        ]
        ...
        self._dict_file_templates = {
            "bdl": Path(self.TEMPLATES_FOLDER, "mut_template_bdl.xlsx"),
            "bdw": Path(self.TEMPLATES_FOLDER, "mut_template_bdl.xlsx"),
            "skt": Path(self.TEMPLATES_FOLDER, "mut_template_psd.psd"),
        }
        ...
        self._edl_episode_prefix = "mut_"
        self.edl_sequence_regex = compile(r"(?<=mut_)\d{3}_\d{3}")
        self._edl_shot_regex = compile(r"(?<=mut_)\d{3}_\d{3}_\d{3}")
        self.edl_version_regex = compile(r"(?<=_[VvWw])\d\d\d")
        self._shot_task_tpl_id = 1069
        self._episode_edl_workflow = True
        self._edl_episode_regex = compile(r"mut_\d{3}")
    ...
    def return_next_version_name(self, ext: Iterable, version: int = None) -> dict:
        ...
        if task is not None:
            ...
            match task.entity_type:
                case "Asset":
                    file_name = f"mut_{type_}_{link}_{task_name}_w{v+1:03}"
                case "Shot":
                    file_name = f"mut_{link}_{task_name}_w{v+1:03}"
                case "Episode":
                    file_name = f"mut_{link}_{task_name}_w{v+1:03}"
    ...
    def return_next_publish_file(self, work_file: Path, task: BaseTask = None):
        ...
        match task.entity_type:
            case "Asset":
                file_name = f"mut_{type_}_{link}_{task_name}_v{v+1:03}"
            case "Shot":
                file_name = f"mut_{link}_{task_name}_v{v+1:03}"
            case "Episode":
                file_name = f"mut_{link}_{task_name}_v{v+1:03}"
